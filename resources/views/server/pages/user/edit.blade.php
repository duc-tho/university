    ...
    <div class="col-sm-12 px-3">
        ...
        <x-admin.form.form method="POST" :cancelLink="route('admin.user.show', [$khoa['slug']])" :submitLink="route('admin.user.update', [$khoa['slug'], $user['id']])">
            <div class="row">
                ...
                <x-admin.form.checkbox :data="$user" :col="3" :label="'Trạng thái'" :required="false" :fieldName="'status'" :labelContent="'Đang hoạt động'" :readonly="true" />
                @else
                ...
                <x-admin.form.multi-select :col="6" :label="'Vai trò'" :required="false" :fieldName="'role'" :readonly="Auth::user()['id'] == $user['id'] ? true : false">
                    @if (Auth::user()['id'] == $user['id'])
