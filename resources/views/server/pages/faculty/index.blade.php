                                ...
                                <div class="input-group input-group-sm">
                                    <form action="{{ route('admin.faculty.show', [$khoa['slug']]) }}" method="get">
                                        <select class="form-control custon-select" style="width:auto !important" data-toggle="tooltip" name="item-per-page" onchange="this.parentElement.submit();">
                                            <option value="6" {{ $faculty_list->perPage() == 6 ? 'selected disabled' : ''  }}>Hiện 6 mục</option>
                                            <option value="9" {{ $faculty_list->perPage() == 9 ? 'selected disabled' : ''  }}>Hiện 9 mục</option>
                                            <option value="12" {{ $faculty_list->perPage() == 12 ? 'selected disabled' : ''  }}>Hiện 12 mục</option>
                                        </select>
                                    </form>
                                </div>
                            </div>
                                        ...
                                        <div class="card bg-light">
                                            ...
                                            <div class="card-body pt-0">
                                                <div class="row">
                                                    <div class="col-7">
                                                        ...
                                                        <p class="text-muted text-sm"><b>Tạo ngày : </b> {{ ConvertDatabaseTimeToDMY($faculty['created_at']) }} </p>
                                            ...
                                            <div class="card-footer">
                                                <div class="text-right">
                                                    <a href="{{ route('admin.faculty.edit', [$khoa['slug'], $faculty['id']]) }}" class="btn btn-sm bg-teal" >
                                                        <i class="fas fa-user-edit" ></i> Sửa
                                                    </a>
                                                    <a href="{{ route('admin.faculty.delete', [$khoa['slug'], $faculty['id']]) }}" class="btn btn-sm btn-danger" onclick="return confirm('Bạn có chắc chắn muốn xóa !')">
                                ...
                                <div class="row">
                                    <div class="col-sm-5 hidden-xs">
                                        <div class="dataTables_info" id="example-datatable_info" role="status" aria-live="polite">
                                            <strong>Trang {{ $faculty_list->currentPage() }} / {{ $faculty_list->lastPage() }}</strong>
                                        </div>
                                    </div>
