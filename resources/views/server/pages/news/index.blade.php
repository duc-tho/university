                                ...
                                <div class="input-group input-group-sm">
                                    <form action="{{ route('admin.news.show', [$khoa['slug']]) }}" method="get">
                                        <select class="form-control custon-select" style="width:auto !important"
                                            ...
                                            <option value="6"
                                                {{ $news_list->perPage() == 6 ? 'selected disabled' : '' }}>Hiện 6 mục
                                            </option>
                                            <option value="9"
                                                {{ $news_list->perPage() == 9 ? 'selected disabled' : '' }}>Hiện 9 mục
                                            </option>
                                            <option value="12"
                                            ...
                                            <div class="col-12 col-sm-6 col-md-4 d-flex align-items-stretch">
                                                <div class="card bg-light">
                                                    ...
                                                    <div class="card-body pt-0">
                                                        <div class="row">
                                                            <div class="col-7">
                                                                <h2 class="lead text-max"><b>{{ $news->title }}</b></h2>
                                                                <p class="text-muted text-sm text-max"><b>Tạo ngày : </b>
                                                                    {{ ConvertDatabaseTimeToDMY($news['created_at']) }}
                                                                </p>
                                                                <ul class="ml-4 mb-0 fa-ul text-muted text-max" >
                                                                    <li class="small"><span class="fa-li"><i
                                                                                class="fas fa-chalkboard-teacher"></i></span>Danh
                                                                        Mục:
                                                                        {{ $category->title }}</li>
                                                                    <br>
                                                                    <li class="small"><span class="fa-li"><i
                                                                                class="fas fa-dice-d20"></i></span>Trạng
                                                                        Thái:
                                                                        @switch($news->status)
                                                                            @case(1)
                                                            ...
                                                            <div class="col-5 text-center">
                                                                <img src="{{ asset($news->image ?? 'dist/img/imgdefault.png') }}" alt="img" class="img-circle img-fluid img-max"  >
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="card-footer">
                                                        <div class="text-right">
                                                            <a href="{{ route('admin.news.edit', [$khoa['slug'], $news['id']]) }}"
                                                                class="btn btn-sm bg-teal">
                                                                <i class="fas fa-user-edit"></i> Sửa
                                                            </a>
                                                            <a href="{{ route('admin.news.delete', [$khoa['slug'], $news['id']]) }}"
                            ...
                            <div class="row">
                                <div class="col-sm-5 hidden-xs">
                                    <div class="dataTables_info" id="example-datatable_info" role="status"
                                        aria-live="polite">
                                        <strong>Trang {{ $news_list->currentPage() }} /
                                            {{ $news_list->lastPage() }}</strong>
                                    </div>
                                </div>
