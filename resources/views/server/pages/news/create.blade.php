...
<div class="row">
    <div class="col-sm-12 px-3">
        <x-admin.form.form method="POST" :cancelLink="route('admin.news.show', [$khoa['slug']])" :submitLink="route('admin.news.store', [$khoa['slug']])">
            <div class="row">
                ...
                <x-admin.form.select :required="true" :col="3" :fieldName="'category_id'" :label="'Chọn Danh Mục :'">
                    ...
                    <option value="{{$category->id}}" {{ old('category_id') == $category->id ? "selected" : '' }}>{{$category->title}} -- {{$faculty->name}}</option>
                    @endif
                    @endforeach
                    @endif
                    @endforeach
                ...
                <x-admin.form.input :col="3" :type="'text'" :label="'Meta_Descriptions'" :required="false" :placeholder="'Meta Descriptions'" :fieldName="'meta_descriptions'" />
                <x-admin.form.input :col="3" :type="'text'" :label="'Meta_Keywords'" :required="false" :placeholder="'Meta Keywords'" :fieldName="'meta_keywords'" />
