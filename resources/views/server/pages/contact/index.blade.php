                                ...
                                <div class="input-group input-group-sm">
                                    <form action="{{ route('admin.contact.show', [$khoa['slug']]) }}" method="get">
                                        <select class="form-control custon-select" style="width:auto !important"
                                            ...
                                            <option value="6"
                                                {{ $contact_list->perPage() == 6 ? 'selected disabled' : '' }}>Hiện 6 mục
                                            </option>
                                            <option value="9"
                                                {{ $contact_list->perPage() == 9 ? 'selected disabled' : '' }}>Hiện 9 mục
                                            </option>
                                            <option value="12"
                                                {{ $contact_list->perPage() == 12 ? 'selected disabled' : '' }}>Hiện 12
                                                mục</option>
                                        </select>
                                    </form>
                                </div>
                                        ...
                                        <div class="card bg-light">
                                            ...
                                            <div class="card-footer">
                                                <div class="text-right">
                                                    <a href="{{ route('admin.contact.edit', [$khoa['slug'], $contact['id']]) }}"
                                                        class="btn btn-sm bg-teal">
                                                        <i class="fas fa-user-edit"></i> Sửa
                                                    </a>
                                                    <a href="{{ route('admin.contact.delete', [$khoa['slug'], $contact['id']]) }}"
                            ...
                            <div class="row">
                                <div class="col-sm-5 hidden-xs">
                                    <div class="dataTables_info" id="example-datatable_info" role="status"
                                        aria-live="polite">
                                        <strong>Trang {{ $contact_list->currentPage() }} /
                                            {{ $contact_list->lastPage() }}</strong>
                                    </div>
                                </div>
